...
$(function() {
	let thead = '<thead><tr><th>No</th><th>depth2</th><th>depth3</th><th>depth4</th><th>depth etc.</th><th>화면명</th><th>파일(화면ID)</th><th>범위</th><th>최근수정일</th><th>작업자</th><th>비고</th></tr></thead>';
	...
	imageList();
	matchMenu();
	if($('.gnb span.active').text() == '2026_상시') uiMYDList('2026_상시'); //2025 상시 리스트
...
function menu(){
	...
	$('.gnb span').on('click', function() {
		let val = $(this).text();
		let _prev = $(this).parents('.gnb').find('.active').text(); //2025 상시
		if ($('.cmImages').length > 0) { //imgList의 경우
			...
			let _menu = val; //2026_상시
			if (val == '전체보기') {
				$('.display').css('display', 'block');
				//$('#fromDate').val('');
			} else if (val == '2026_상시'){ //2026_상시
				$('.display').css('display', 'block');
			} else {
				if(_prev == '전체보기') _menu = _prev; //2026_상시
...
//2026_상시 : 2026_상시 탭
function uiMYDList(_val){
	if(_val == '전체보기') $('.display tbody tr').css('display', '');
	else if(_val == '2026_상시') $('.display tbody tr').css('display', 'none').siblings('.new, .mod').css('display', '');
}

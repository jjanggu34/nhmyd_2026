...
$(function() {
	...
	imageList();
	matchMenu();
	if($('.gnb span.active').text() == '2025_상시') uiMYDList('2025_상시'); //2025 상시 리스트
...
function menu(){
	...
	$('.gnb span').on('click', function() {
		...
		if ($('.cmImages').length > 0) { //imgList의 경우
			...
			let _menu = val; //2025_상시
			if (val == '전체보기') {
				$('.display').css('display', 'block');
				//$('#fromDate').val('');
			} else if (val == '2025_상시'){ //2025_상시
				$('.display').css('display', 'block');
			} else {
...
function uiMYDList(_val){
	...
	else if(_val == '2025_상시') $('.display tbody tr').css('display', 'none').siblings('.new, .mod').css('display', '');
}
